      ...
      <div className="home">
        ...
        {props.navBarData.menus.find((menu: any) => menu.id === 1).enabled &&
          <section id="hero-area" className="bg-bottom py-0">
            <Parallax bgImage={`${api.baseUrl}${props.homeData.backgroundImage.url}`} strength={300}>
        ...
        <div className="float-text">
          <div className="de_social-icons">
            {props.mediaSocialData.map((item: any) => (
              <a href={item.link} target="_blank" rel="noopener noreferrer" key={item.id}><span className="button"><i className={item.iconClassName}></i></span></a>
            ))}
          </div>
          <span>Follow Me</span>
        </div>
